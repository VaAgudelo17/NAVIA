...
function normalizeForTts(text: string): string {
  return text
    ...
    // Abreviaturas técnicas generales
    .replace(/\bIA\b/g, 'inteligencia artificial')
    .replace(/\bOCR\b/g, 'reconocimiento óptico de texto')
    .replace(/\bUSB\b/gi, 'u ese be')
    .replace(/\bCPU\b/gi, 'ce pe u')
    .replace(/\bGPU\b/gi, 'ge pe u')
    .replace(/\bLCD\b/gi, 'ele ce de')
    .replace(/\bHDMI\b/gi, 'hache de eme i')
    .replace(/\bSIM\b/gi, 'sim')
    .replace(/\bSSD\b/gi, 'ese ese de')
    .replace(/\bHDD\b/gi, 'hache de de')
    // Formatos de archivo
    .replace(/\bJPEG\b/gi, 'jota pe e ge')
    .replace(/\bJPG\b/gi, 'jota pe ge')
    .replace(/\bPNG\b/gi, 'pe ene ge')
    .replace(/\bPDF\b/gi, 'pe de efe')
    .replace(/\bMP3\b/gi, 'eme pe tres')
    .replace(/\bMP4\b/gi, 'eme pe cuatro')
    ...
    // Unidades de almacenamiento digital (número+unidad primero, luego standalone)
    .replace(/(\d+(?:[.,]\d+)?)\s*TB\b/gi, '$1 terabytes')
    .replace(/(\d+(?:[.,]\d+)?)\s*GB\b/gi, '$1 gigabytes')
    .replace(/(\d+(?:[.,]\d+)?)\s*MB\b/gi, '$1 megabytes')
    .replace(/(\d+(?:[.,]\d+)?)\s*KB\b/gi, '$1 kilobytes')
    .replace(/\bTB\b/gi, 'terabytes')
    .replace(/\bGB\b/gi, 'gigabytes')
    .replace(/\bMB\b/gi, 'megabytes')
    .replace(/\bKB\b/gi, 'kilobytes')
    // Frecuencias
    .replace(/(\d+(?:[.,]\d+)?)\s*GHz\b/gi, '$1 gigahercios')
    .replace(/(\d+(?:[.,]\d+)?)\s*MHz\b/gi, '$1 megahercios')
    .replace(/(\d+(?:[.,]\d+)?)\s*Hz\b/gi, '$1 hercios')
    // Unidades de distancia / dimensión
    .replace(/(\d+(?:[.,]\d+)?)\s*km\/h\b/gi, '$1 kilómetros por hora')
